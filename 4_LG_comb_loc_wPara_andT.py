...
a_new_par = np.zeros((626, 3201, 7)) # initialize new data array for 5 parameters columns and 2 co-ordinate columns per data point per time step
...
a_T = np.load("a_T_clean_1.npy")
print('Scaling data...')
print( a_T.shape[-1], a_T.shape, a_x_loc.shape)
# the first reshape makes the 3D to 2D array with same enteries
a_transform = (a_T.reshape(-1, a_T.shape[-1])).T
a_T_sc1 = (scaler.fit_transform(a_transform)).T
a_T_sc  = a_T_sc1.reshape(a_T.shape)
